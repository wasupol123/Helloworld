...
int main()
{
	...
	/*
		if(LL_TIM_IsActiveFlag_UPDATE(TIM2) == SET)
		{
			if(state)
			{
				LL_TIM_DisableCounter(TIM4); //Change ARR of Timer PWM
				LL_TIM_SetAutoReload(TIM2,200); //Change ARR of Timer PWM
				state = 0;
			}
			else
		  {
				LL_TIM_SetAutoReload(TIM4, ARR_CALCULATE(sheetnote[i])); //Change ARR of Timer PWM
				LL_TIM_EnableCounter(TIM4);
				LL_TIM_SetAutoReload(TIM2, 800); //Change ARR of Timer PWM
				state = 1;				
			}		
			++i;
			LL_TIM_ClearFlag_UPDATE(TIM2);
